...
- **Sensitivity**: All compliance reports and certificates must be marked CONFIDENTIAL — INTERNAL USE ONLY unless otherwise directed.
...
1. **Self-verify**: Run all applicable verification gates:
 - Build/test gates: validation scripts execute without errors, all 10 hard gates checked
...
|-------|----------------|-------------|
| Session Artifact Collection | `artifact_collection.md` | `artifact_collection.json` |
| Hard Gate Validation | `gate_validation.md` | `gate_validation.json` |
| Compliance Assessment | `compliance_assessment.md` | `compliance_assessment.json` |
| Certificate Generation | `certificate.md` | `certificate.json` |
...
```json
{
 ...
 "skill_id": "devinclaw.sdlc_validator.v1",
 "artifacts": [
 {
...
- **Human approval required for**: compliance certification evidence package compilation, cross-boundary session validation, certificate issuance for safety-critical systems.
