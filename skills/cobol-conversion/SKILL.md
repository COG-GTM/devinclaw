...
The organization's mission-critical systems and supporting administrative systems contain COBOL programs that have been in production for 30-40 years. These programs encode critical business rules for operations management, personnel systems, financial processing, and regulatory compliance. The COBOL developer workforce is aging out, making modernization urgent.

## What's Needed From User

- **COBOL source files**: The COBOL programs (.cbl, .cob) and all referenced copybooks (.cpy) to convert.
- **Target language**: Java (default), Python, or TypeScript.
- **COBOL dialect**: Which COBOL dialect is in use (IBM Enterprise COBOL, Micro Focus COBOL, ACUCOBOL, or GnuCOBOL).
- **Runtime environment**: Mainframe (z/OS), midrange (AS/400), or distributed (Linux/Windows).
- **Inter-program calls**: List of called subprograms (CALL statements) and whether they are in scope for conversion.
- **File and database access**: Description of file I/O (sequential, VSAM, ISAM) and database access (DB2 EXEC SQL, IMS DL/I) patterns.
- **JCL or job documentation** (optional): Job control language or scheduling documentation that shows how programs are invoked and their execution sequence.
- **Test data** (optional): Sample input files, expected output files, and database test fixtures.
...
1. **Self-verify**: Run all applicable verification gates:
 - Build/test gates: target language compilation, unit tests, numeric precision validation
...
|-------|----------------|-------------|
| Copybook Analysis | `copybook_analysis.md` | `copybook_analysis.json` |
| Program Inventory | `program_inventory.md` | `program_inventory.json` |
| Conversion Specification | `conversion_spec.md` | `conversion_spec.json` |
| Code Generation | `code_generation.md` | `code_generation.json` |
| Equivalence Testing | `equivalence_test.md` | `equivalence_test.json` |
...
```json
{
 ...
 "skill_id": "devinclaw.cobol_conversion.v1",
 "artifacts": [
 {
...
- **Human approval required for**: packed decimal precision strategy, CICS transaction mapping decisions, batch job scheduling changes.
