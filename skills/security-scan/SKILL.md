---
name: security-scan
description: Scan codebases for CIS Benchmark, SOC 2 / ISO 27001, and cloud security compliance compliance violations. Use this skill when performing enterprise security audits on enterprise applications, when validating compliance posture before compliance certification submissions, or when scanning for vulnerabilities and security misconfigurations in modernized code.
---
...
- **System categorization**: security tier -- Low, Moderate, or High. This determines which baselines apply.
- **cloud security compliance baseline** (optional): If the application is security-certified, specify the baseline (Low, Moderate, High, or LI-SaaS). If not provided, the security categorization is used to select the equivalent baseline.
...
- **Existing compliance package** (optional): Links to current System Security Plan (SSP), Plan of Action and Milestones (remediation plan), and previous scan results for delta analysis.
- **Scan scope** (optional): Full scan (default) or targeted scan of specific benchmark IDs, security control families, or code directories.
...
1. **Self-verify**: Run all applicable verification gates:
 - Build/test gates: remediation code compilation, security test suite execution
 - Security gates: re-scan CIS Benchmark benchmarks post-remediation, dependency CVE re-scan, secrets scan
 - Compliance gates: compliance report result validation, OSCAL format verification
...
|-------|----------------|-------------|
| CIS Benchmark Scan | `benchmark_scan.md` | `benchmark_scan.json` |
| SOC 2 / ISO 27001 Assessment | `security_assessment.md` | `security_assessment.json` |
| Cloud Security Validation | `cloud_security_validation.md` | `cloud_security_validation.json` |
| Zero Trust Assessment | `zero_trust.md` | `zero_trust.json` |
| Remediation SDD | `remediation_sdd.md` | `remediation_sdd.json` |
...
- **Human approval required for**: Critical severity finding risk acceptance, compliance package submission, safety-critical code changes (DO-178C DAL A-C), remediation plan entry creation.
